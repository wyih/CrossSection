...
comp0 = fread('../DataRaw/CompustatAnnual.csv') %>%
  as.data.frame()  %>%
  mutate(
    naicsstr = str_pad(
    ...
    , year_avail = year(as.Date(datadate) %m+% months(6) )+1
  ) %>%
  filter(!is.na(naics6)) %>%
  select(gvkey, year_avail, naics6, datadate)

# read crsp
crsp0 = fread('../DataRaw/mCRSP.csv') %>%
  transmute(
    permno,date,ret=100*ret,mve_c = abs(prc)*shrout
  ) %>%
...
ccm0  = fread('../DataRaw/CCMLinkingTable.csv') %>%
  transmute(
    gvkey
    , permno = lpermno
    , linkprim
...
### SAVE
data.table::fwrite(iomom, file = '../DataRaw/InputOutputMomentum.csv') 
